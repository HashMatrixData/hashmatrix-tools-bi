# CLAUDE.md — hashmatrix-tools-bi 协作与合规指引
...
**本仓视角（tools-bi）**：报表 / 可视编排 / 数据大屏**按租户隔离取数**；集成的 BI 设计器（如 DataEase）纳入时保持租户上下文，并以**部署级品牌**做品牌化外框（不按租户换肤）。

> 全局定义见主仓 `docs/00-主仓初始化-spec.md` 与 `docs/architecture/05-多租户与控制平面.md`。

## 仓库定位

数据工具分系统：报表 / 自助分析 / 可视化、可视化编排。
